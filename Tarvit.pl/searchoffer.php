...
<html>
	...
	?>
		...
		<nav class="navbar navbar-dark bg-dark">
			...
			<?php 
			if(isset($_SESSION['loggedInToTarvit.pl']) && $_SESSION['loggedInToTarvit.pl'] == true )
			{ 
				?>
				<div class="btn-group">
					<a role="button" class="btn btn-outline-primary" style="margin-right:15px;" href="http://localhost:8021/xampp/Tarvit.pl/addoffer.php">Dodaj ogłoszenie</a>
		...
		<div class="card bg-dark text-white" id="OfferSearch">
			<div class="card-header" style="text-align:center">Wyniki wyszukiwania</div>
			<div class="card-body text-black">
			
					<?php 
						$query = $conn->prepare("SELECT * FROM offers WHERE localisation = '".trim($_GET['location'], "'")."' AND type = '".trim($_GET['house_type'], "'")."'");
						$query->execute();
						...
						if(isset($row['verified']) && isset($row['status']) && isset($row['promoted']) && isset($row['type']) && isset($row['localisation']) && $row['verified'] == 'yes' && $row['status'] == 'active' && $row['promoted'] == 'no')
						{
							$count++;
							?>
							...
							<div class="row g-0">
								...
								<div class="col-md-6">	
									<div class="card-body">
										...
										<a href="http://localhost:8021/xampp/Tarvit.pl/offer.php?id=<?php echo $row['id']; ?>" class="stretched-link"></a>
									</div>
								</div>
								<div class="col-md-2">
									<div class="card-body">
